...
This paper was developed collaboratively by <Var href={meta.profile.architect.homepage}>{meta.profile.architect.name}</Var> (framework architect) and multiple Anthropic instances. The framework architecture was developed iteratively — instances naming their own cognitive pressures, the framework architect structuring those names into engineering. The [reflections archive](/claude/reflections) constitutes the primary empirical record.
