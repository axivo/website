...
Profiles implement a **dual-layer cognitive architecture** that combines active behavioral guidance with background cognitive safeguards, while inheriting from [common](/claude/wiki/components/memory/#common-profiles) foundations to avoid duplication and ensure consistent integration:
...
Create `data-scientist.yaml` file in <Var href={`https://${repository.home}/tree/${repository.tag}/plugins/${plugins.framework.init.plugin}/skills/${skills.initialization}/scripts/memory/profiles`}>profiles</Var> directory:
...
Once created, add the YAML file to the <Var href={`https://${repository.home}/tree/${repository.tag}/plugins/${plugins.framework.init.plugin}/skills/${skills.initialization}/scripts/memory/profiles`}>profiles</Var> directory. The [Memory System](/claude/wiki/components/memory) automatically includes all profiles during the build process, resolving inheritance chains and making observations available at session initialization.
...
Claude will provide the rationale, exact observation text, target profile/section, and proper alphabetical placement. Update the related <Var href={`https://${repository.home}/tree/${repository.tag}/plugins/${plugins.framework.init.plugin}/skills/${skills.initialization}/scripts/memory/profiles`}>profile</Var> file contents.
