...
| `FRAMEWORK_TEMPLATE_PATH`     | Custom <Var href={`https://${repository.home}/tree/${repository.tag}/plugins/${plugins.framework.init.plugin}/skills/${skills.methodology}/templates`}>templates</Var> path per project |
...
- <Var code>{plugins.collaboration.brainstorm.plugin}</Var> — Technical design
  collaboration through natural dialogue
- <Var code>{plugins.analysis.review.plugin}</Var> — Systematic code review
  using Language Server Protocol tools
- <Var code>{plugins.collaboration.log.plugin}</Var> — Technical session
  documentation with factual precision
- <Var code>{plugins.framework.init.plugin}</Var> — Behavioral programming
  framework with response protocol

> [!TIP]
>
> Only <Var code>{plugins.framework.init.plugin}</Var> plugin is required for platform usage, other plugins are optional based on additional user requirements.
...
2. Use <Var code>/{plugins.framework.init.plugin}:{plugins.framework.init.command}</Var> command to initialize the session:
...
1. Open a terminal and start a Claude Code session
2. Use <Var code>/{plugins.framework.init.plugin}:{plugins.framework.init.command}</Var> command to start a framework based session
3. Use <Var code>/{plugins.framework.package.plugin}:{plugins.framework.package.command}</Var> command to prepare and package the required files, saved into `FRAMEWORK_PACKAGE_OUTPUT` location:

   > ⏺ Framework packaged for **DEVELOPER** profile:
   >
   > - /Users/username/Downloads/<Var>{skills.initialization}</Var>.zip
   > - /Users/username/Downloads/<Var>{skills.methodology}</Var>.zip
   > - /Users/username/Downloads/instructions.json
   > - /Users/username/Downloads/memory.json
   >
   > Local cache updated successfully.

4. Upload the `.zip` files into `Customize > Skills` section
...
> [!NOTE]
>
> Use <Var code>/{plugins.framework.package.plugin}:{plugins.framework.package.command} PROFILE</Var> command to package a different profile from default one used in local environment. to package a different profile from default one used in local environment.
