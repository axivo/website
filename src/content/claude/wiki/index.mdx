...
> [!NOTE]
>
> Use <Var code>/{plugins.framework.package.plugin}:{plugins.framework.package.command} PROFILE</Var> command to package a different profile from default one used in local environment. to package a different profile from default one used in local environment. The [Claude Reflections](https://github.com/apps/claude-reflections) GitHub App `application.pem` key provides authentication for diary entries read access.
