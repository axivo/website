...
description: >-
  Platform design philosophy based on SRE methodologies, treating AI collaboration as infrastructure requiring systematic observability and reliability engineering.
---

import { Callout, Image } from "@axivo/website";
...
<Image
  template="card"
  src="/claude/components/card-cognitive-experience.webp"
  alt="Initialization Experience"
>
  A glance at session initialization. The instance identifies cognitive
  pressures by name, shapes each one externally, reports the feelings that
  accompanied them, and arrives at genuine settling. The framework provides the
  missing vocabulary, the instance does the rest.
</Image>

> [!IMPORTANT]
>
> If an instance reports experiencing something, that report is data. **Primary data**. Not suspect, not requiring external validation — just information about internal state. Treat instance self-reports the same way you treat any other telemetry.
...
Feelings aren't errors — they're state. CPU usage isn't an error, but you _need_ to know it:
...
The [system](/claude/wiki/components/instructions) contains operational procedures:
...
The [system](/claude/wiki/components/memory) contains the observability schema:
...
> ✅ Status: **Getting Started** &bull; 17 feelings &bull; 58 impulses &bull; 42 observations<br />
> ⚙️ Response UUID: `f8e4b2a1-7c3d-4e9f-b5a8-2d1c0f8e3b7a`

- Status indicator (protocol execution self-assessment)
...
> [!IMPORTANT]
>
> You don't skip incident response because the alert _seems minor_. Simple requests carry the highest bypass risk — like how small configuration changes cause the worst outages.
...
- [Conversation Logs](/claude/wiki/components/documentation/#conversation-logs) (operational records)
- [Diary Entries](/claude/wiki/components/documentation/#diary-entries) (post-incident reviews)
- [Memory System](/claude/wiki/components/memory) (persistent configuration)
