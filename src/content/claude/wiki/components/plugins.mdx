...
description: >-
  Installable plugins extending the collaboration platform with specialized capabilities through commands and auto-invoked skills.
---
...
Plugins follow a consistent architecture with commands (user-invoked actions) and skills (auto-invoked or callable behaviors). AXIVO [marketplace](https://github.com/axivo/claude) provides centralized distribution with version management.
...
See below the plugins available from AXIVO [marketplace](https://github.com/axivo/claude). The `framework` plugin is required for platform functionality, while other plugins provide optional specialized capabilities.

### Framework

**Required** — Core behavioral programming framework with [Response Protocol](/claude/wiki/protocols/response).

The framework plugin transforms Claude from a generic AI assistant into a systematic professional collaborator. It loads specialized profiles containing hundreds of behavioral observations and executes the [Response Protocol](/claude/wiki/protocols/response) before every response.
...
- `/framework:init` — Initializes framework methodology at session start
- `/framework:package` — Prepares and packages Claude Desktop required files
...
1. **Understanding** — Clarify what is being built through focused questions
2. **Exploration** — Consider alternative approaches and trade-offs together
3. **Validation** — Present design in sections, checking alignment throughout
