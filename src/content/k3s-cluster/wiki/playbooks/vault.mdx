...
title: Vault
description: >-
  Ansible playbook for managing Vault encrypted variables and updating the global password.
---
...
The playbook allows the end-user to perform various Ansible Vault related tasks, such as listing currently encrypted role variables, encrypting a specific role variable, and updating the Ansible Vault global password.
...
The newly encrypted variables will be displayed — update the <Var code href={`https://${repository.home}/blob/${repository.tag}/inventory/cluster/group_vars/all.yaml`}>all.yaml</Var> group variables file with the new values.
